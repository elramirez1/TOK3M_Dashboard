...
def get_graficos(
    ...
    ejecutivos: Optional[List[str]] = Query(None) # <--- Nuevo Filtro
):
    try:
        ...
        if ejecutivos: # <--- Lógica del nuevo filtro
            eje_str = ",".join([f"'{e}'" for e in ejecutivos])
            filters.append(f"NOMBRE_EJECUTIVO IN ({eje_str})")
