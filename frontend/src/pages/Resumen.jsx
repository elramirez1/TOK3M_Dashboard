...
// Componente para renderizar nombres con saltos de línea
const CustomYAxisTick = (props) => {
  const { x, y, payload } = props;
  const name = payload.value;
  const words = name.split(' ');
  
  // Si el nombre tiene más de 2 palabras, lo dividimos
  if (words.length > 2) {
  ...
  return (
    <g transform={`translate(${x},${y})`}>
      <text x={-10} y={4} textAnchor="end" fill="#9CA3AF" fontSize={9} fontWeight="bold">
        {name}
      </text>
    </g>
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[900px]">
        ...
        <div className="bg-[#111827] p-10 rounded-[2.5rem] border border-gray-800 flex flex-col shadow-xl overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto pr-4 custom-scrollbar">
            <ResponsiveContainer width="100%" height={Math.max(800, (graficos.por_ejecutivo?.length || 0) * 55)}>
              <BarChart data={graficos.por_ejecutivo || []} layout="vertical" margin={{ left: 60, right: 20 }}>
                <YAxis 
        ...
        <div className="flex flex-col gap-8 h-full">
          ...
          <div className="bg-[#111827] p-8 rounded-[2.5rem] border border-gray-800 flex-1 shadow-xl flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto pr-4 custom-scrollbar">
              <ResponsiveContainer width="100%" height={Math.max(300, graficos.por_empresa.length * 35)}>
